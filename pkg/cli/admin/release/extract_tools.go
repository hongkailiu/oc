...
func findClusterIncludeConfig(ctx context.Context, restConfig *rest.Config) (manifestInclusionConfiguration, error) {
	...
	if clusterVersion, err := client.ClusterVersions().Get(ctx, "version", metav1.GetOptions{}); err != nil {
		...
		// this way to collect the complete set of enabled capabilities. Only a small amount of capabilities are added over time
		// and that happens only for minor level update: #C(4.11)=4 -> #C(4.17)=15, averagely less than two per minor update.
		// https://docs.openshift.com/container-platform/4.17/installing/overview/cluster-capabilities.html
		// It might not be worth the effort to calculate the exact enabled capabilities.
		// As a result of allowing unknown capabilities, a few unexpected manifest could be included.
